...
$(XPIDL_GEN_DIR)/%.idlcsv: %.idl $(DXRSRC)/plugins/moztools/idl_xref.py
	$(PYTHON) $(xpidldir)/config/pythonpath.py \
		-I$(xpidldir)/other-licenses/ply \
		-I$(xpidldir)/xpcom/idl-parser \
		$(DXRSRC)/plugins/moztools/idl_xref.py --cachedir=$(xpidldir)/xpcom/idl-parser $(XPIDL_FLAGS) $(_VPATH_SRCS) > $@
